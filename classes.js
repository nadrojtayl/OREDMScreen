...
class NPC{
	constructor(name,backstory){
		this.el = $("<div></div>")
		this.el.append($("<h5>"+ name +"</h5>"));
		this.el.append($("<p>"+backstory +"</p>"));
		this.el.append($("<textarea></textarea>"))
		this.el.css({"border-color": "black", 
             "border-width":"1px", 
             "border-style":"solid"});
		this.creature = new Creature(5,10,8,"","Attack",6,3);

	}

	render(div){
		this.creature.render(this.el);
		console.log(div);
		div.append(this.el);
	}
}
...
class NPCHolder{
	...
	addNPC(name,backstory){
		this.npcs.push(new NPC(name,backstory))
		this.render();
	}
	render(div){
		var that = this;
